...
from ..schemas.book import (BookCreate, BookUpdate, BookFilters, ShowBook,)
from ..schemas.common import PaginatedResponse, PaginationParams, PaginatedResponse
...
async def get_books(service: BookServiceDep, pagination: Annotated[PaginationParams, Depends()], title: str | None = Query(None, description="Поиск по названию"),
    ...
    books, total = await service.search_books(title=title,
        author=author,
        genre=genre,
        year=year,
        available=available,
...
@router.get("/{book_id}", response_model=ShowBook, summary="Получить книгу", description="Получить информацию о конкретной книге по ID")
...
@router.patch('/{book_id}',response_model=ShowBook, summary='Обновить книгу', description="Частичное обновление книги (передаются только изменяемые поля)")
...
@router.delete('/{book_id}', status_code=status.HTTP_204_NO_CONTENT, summary="Удалить книгу", description="Удалить книгу из каталога")
